...
@help.command(aliases=['command', 'commands'])
async def cmd(ctx):
    em = discord.Embed(
...
async def commandlist(ctx):
    ...
    em.add_field(name=":information_source: Info", value="1 Command", inline=False)
    em.add_field(name=":moneybag: Economy", value="1 Command", inline=True)
    em.add_field(name=":wrench: Utility", value="1 Command", inline=False)
    em.add_field(name=":8ball: Fun", value="1 Command", inline=True)
    em.timestamp = datetime.datetime.utcnow()
    em.set_author(name=client.user.name, url="", icon_url=client.user.avatar.url)
    em.set_footer(text=f"Requested by {ctx.author.display_name}", icon_url=ctx.author.avatar.url)
